...
/*
 * In the UM kernel, configuration is provided by the host.
 * It can be fetched via the host interface. Here we reserve a buffer
 * of 4096 bytes, and read configuration from there.
 */

static char confbuf[4096];

#define CONFADDR confbuf
...
print("userinit pc=%08lux, sp=%08lux\n", p->sched.pc, p->sched.sp);
	/*
	 * User Stack
	 *
...
print("got here in userinit USTKTOP=%08lux\n", USTKTOP);
	p->seg[SSEG] = s;
...
confinit(void)
{
	...
	if(conf.nproc > 2000)
		conf.nproc = 2000;
	conf.nimage = 200;
	conf.nswap = 0;			// although swap stuff remains in
	conf.nswppo = 0;		// swap itself is disabled
