...
/*
 * For um, set absolute addresses here, and um_main as entry point
 */

TEXT _multibootheader(SB), $0
	LONG	$0x1BADB002					/* magic */
	LONG	$0x00010001					/* flags */
	LONG	$-(0x1BADB002 + 0x00010001)	/* checksum */
	LONG	$_multibootheader(SB)		/* header_addr */
	LONG	$_startKADDR(SB)			/* load_addr */
	LONG	$edata(SB)					/* load_end_addr */
	LONG	$end(SB)					/* bss_end_addr */
	LONG	$um_main(SB)				/* entry_addr */
	LONG	$0							/* mode_type */
	LONG	$0							/* width */
	LONG	$0							/* height */
	LONG	$0							/* depth */
...
/*
 * UM kernel cannot use STI/CLI, so these functions are moved
 * to umhost.c
 */
